-- VHDL Entity BCtr_lib.temp_avg.interface
--
-- Created:
--          by - nort.UNKNOWN (NORT-XPS14)
--          at - 19:28:06 01/24/2018
--
...
--
-- VHDL Architecture BCtr_lib.temp_avg.fsm
--
-- Created:
--          by - nort.UNKNOWN (NORT-XPS14)
--          at - 15:00:10 10/30/2017
--
-- Generated by Mentor Graphics' HDL Designer(TM) 2016.1 (Build 8)
--
...
--   ack          OUT              '0'                         CLKD   
--   AccArray     LOCAL            (others => 34X"000000000")  CLKD   
--   CntArray     LOCAL            (others => X"0000")         CLKD   
--   OutCache     LOCAL            (others => '0')             CLKD   
--   RdProcessed  LOCAL            '0'                         CLKD   
--   
LIBRARY ieee;
USE ieee.std_logic_1164.all;
USE ieee.numeric_std.all;
...
BEGIN
  ...
  Variable AccTemp : std_logic_vector(33 downto 0);
  BEGIN
    IF (rising_edge(clk)) THEN
      ...
      ELSE
        ...
        CASE current_state IS
          ...
          WHEN s1 => 
            IF Sensor <= "101" THEN
              brd_num_i := to_integer(unsigned(Sensor));
              RData_cld <= std_logic_vector(CntArray(brd_num_i));
              CntArray(brd_num_i) <= (others => '0');
              AccTemp := std_logic_vector(AccArray(brd_num_i));
              OutCache <= AccTemp(33 downto 2);
              AccArray(brd_num_i) <= (others => '0');
            END IF;
            RdProcessed <= '1';
          WHEN s2 => 
            IF brd_num > "000" and brd_num <= "110" THEN
              brd_num_i := to_integer(unsigned(brd_num)-1);
              IF rd_data_o(31 downto 29) /= "001" AND
                rd_data_o(31 downto 29) /= "110" THEN
                CntArray(brd_num_i) <=
                  CntArray(brd_num_i) + 1;
                AccArray(brd_num_i) <=
                  AccArray(brd_num_i) +
                   resize(signed(rd_data_o(30 downto 0)),34);
              END IF;
            END IF;
            ack_cld <= '1';
          WHEN s3 => 
